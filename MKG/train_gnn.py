...
class RGCNLinkPredictor(nn.Module):
    def __init__(self, n_entities, n_relations, emb_dim):
        ...
        # RGCNConv 层
        # 我们使用一个两层的 GNN
        # 注意：RGCNConv 自己处理关系 embedding，所以我们不需要单独定义
        self.conv1 = RGCNConv(emb_dim, 200, n_relations)
        self.conv2 = RGCNConv(200, emb_dim, n_relations)
...
def main():
    ...
    output_dir = Path("output/compgcn_model")
    ...
    print("Converting data to PyG graph format...")
    ...
    # --- 初始化 RGCN 模型 ---
    # ✅ 2. 实例化新的 RGCNLinkPredictor
    model = RGCNLinkPredictor(
        n_entities=n_entities,
        n_relations=n_relations * 2, # *2 是因为我们添加了反向关系
    ...
    optimizer = optim.Adam(model.parameters(), lr=1e-3, weight_decay=5e-4) # GNN 通常需要更强的正则化
    
    # --- 训练循环 (与 CompGCN 版本完全相同) ---
    train_triples = torch.stack([source_nodes, relations, target_nodes], dim=1)
    train_dataloader = DataLoader(TensorDataset(train_triples), batch_size=1024, shuffle=True)
    ...
    for epoch in range(n_epochs):
        ...
        for (batch_triples,) in tqdm(train_dataloader, desc=f"Epoch {epoch+1}/{n_epochs}"):
            ...
            pos_h, pos_r, pos_t = batch_triples[:, 0].to(device), batch_triples[:, 1].to(device), batch_triples[:, 2].to(device)
            
            # ✅ 核心修复：在每个 batch 内部重新计算 GNN 的前向传播
            # 这样每次 loss.backward() 都有一个全新的计算图
            z = model.forward_encoder(initial_x, edge_index, edge_type)
            
            # 采样负样本
            # 注意：这里我们只采样坏尾，以简化，你也可以像 RotatE 一样对称地采样
    ...
    final_node_embeddings = model.forward_encoder(initial_x, edge_index, edge_type)
