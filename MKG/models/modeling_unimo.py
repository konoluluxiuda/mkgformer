...
# some function
def get_extended_attention_mask(attention_mask: torch.Tensor, input_shape: tuple, device: torch.device) -> torch.Tensor:
    """
    ...
    # ✅ 修改这一行，用 attention_mask 的 dtype
    extended_attention_mask = extended_attention_mask.to(dtype=attention_mask.dtype, device=device)
...
class UnimoEncoder(nn.Module):
    ...
    def forward(
        ...
        for idx in range(self.vision_config.num_hidden_layers):
            ...
            past_key_values = text_layer_output[-1] if (idx >= 8 and text_layer_output is not None) else None
