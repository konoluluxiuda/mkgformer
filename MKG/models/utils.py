...
class LabelSmoothSoftmaxCEV1(nn.Module):
    def __init__(self, lb_smooth=0.1):  # 修改参数名以匹配 transformer.py 中的调用
        super().__init__()
    ...
    def forward(self, logits, labels):
        ...
        labels: [batch_size] 长整型标签 或 [batch_size, num_classes] one-hot标签
        """
        # 如果 labels 是 one-hot 编码，转换为类别索引
        if labels.dim() == 2:
        ...
        confidence = 1.0 - self.lb_smooth
        low_conf = self.lb_smooth / (num_classes - 1)

        # 生成 [batch_size, num_classes] 平滑标签
        smoothed_labels = torch.full((batch_size, num_classes),
                                     low_conf,
