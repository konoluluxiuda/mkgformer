...
class ModalRotatE(nn.Module):
    def __init__(self, text_embeddings, smiles_embeddings, component_indices_map, 
                 n_relations, emb_dim, 
                 gate_hidden_dim, # ✅ 新增：门控网络隐藏层维度
                 dropout_p=0.0):  # ✅ 新增：Dropout 概率
        super().__init__()
        ...
        self.gate_layer = nn.Sequential(
            nn.Linear(emb_dim * 2, gate_hidden_dim),
            nn.ReLU(),
            nn.Dropout(dropout_p), # 在激活后加入 Dropout
    ...
    def _scoring(self, h_emb, r_emb, t_emb):
        ...
        # ✅ 核心修复：为 sqrt 添加一个 epsilon 以保证数值稳定性
        epsilon = 1e-12
        score_squared = re_score**2 + im_score**2
        
        # 返回 L2 距离
        return torch.sqrt(score_squared + epsilon).sum(dim=-1)

    def forward(self, h_idx, r_idx, t_idx=None, n_idx=None, mode='tail'):
         # ✅ 核心修复 2: 在 forward 开始时就进行归一化
        with torch.no_grad():
...
def evaluate_kge(model, dataset, all_triples_map, device, batch_size=128):
    ...
    for h_idx, t_idx, r_idx in tqdm(DataLoader(dataset, batch_size=batch_size), desc="Evaluating"):
        ...
        scores = model(h_idx=h_idx, r_idx=r_idx) # -> [batch_size, num_entities]
...
def main():
    ...
    output_dir = Path("output/rotate_model_manual")
    ...
    # =============================================================
    # ✅ 核心修改：在这里设置你要测试的模型架构参数
    # =============================================================
    model_params = {
        'gate_hidden_dim': 768 // 8, # 尝试更窄的门控网络 (96)
        'dropout_p': 0       # 尝试加入 0% 的 Dropout
    }
    print(f"Using model architecture parameters: {model_params}")
