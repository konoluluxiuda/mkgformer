...
def _load_rec_data():
    ...
    if not os.path.exists(rec_path):
        raise FileNotFoundError(
            f"未找到 {rec_path}。请先运行 python MKG/recommand/preprocess_kge.py"
        )
...
def main():
    ...
    print(
        f"Data summary: diseases={len(disease_ids)}, herbs={len(herb_indices)}, "
        f"train_queries={len(train_dict)}, test_queries={len(test_dict)}"
    )
    ...
    def _on_iter_end(model_obj, iter_idx, total_iters):
        ...
        results = _evaluate_topk(model_obj, test_dict, herb_indices, Config.top_k)
        res_str = " | ".join([f"{k}: {v:.4f}" for k, v in results.items() if 'F1' in k])
        print(f"Iter {iter_idx}/{total_iters} | Test Metrics: {res_str}")
