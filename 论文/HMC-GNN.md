...
For clarity, the key mathematical notations used throughout this paper are summarized in Table 1. Let ![](data:image/x-wmf;base64...) and ![](data:image/x-wmf;base64...) denote the sets of diseases (or clinical syndromes) and candidate herbs, respectively, where ![](data:image/x-wmf;base64...) and ![](data:image/x-wmf;base64...) represent their total numbers.

To capture complex therapeutic interactions and rich physical modalities, we construct a unified multi-relational heterogeneous graph, defined as ![](data:image/x-wmf;base64...). The node set ![](data:image/x-wmf;base64...) comprises not only the core diseases and herbs (![](data:image/x-wmf;base64...)), but also multi-granularity attribute nodes, denoted as ![](data:image/x-wmf;base64...) (e.g., macroscopic TCM properties and microscopic chemical fingerprints). Crucially, the multi-relational edge set ![](data:image/x-wmf;base64...) encompasses various interaction types ![](data:image/x-wmf;base64...) (e.g., treats\_disease, has\_property, and collaborative\_co\_occurrence). This multi-relational topological nature serves as the fundamental theoretical prerequisite for applying Relational Graph Convolutional Networks (RGCN), explicitly empowering the model to distinguish and aggregate information across entirely different semantic pathways during the message-passing phase.
...
| ![](data:image/x-wmf;base64...) | Set of diseases (syndromes) and candidate herbs                                                                                                            |
| ![](data:image/x-wmf;base64...) | Total number of diseases and candidate herbs                                                                                                               |
| ![](data:image/x-wmf;base64...) | The unified multi-relational heterogeneous TCM graph                                                                                                       |
| ![](data:image/x-wmf;base64...) | Sets of nodes, edges, and relation types in ![](data:image/x-wmf;base64...)                                                                                |
| ![](data:image/x-wmf;base64...) | Set of multi-granularity attribute nodes (TCM properties, chemical fingerprints)                                                                           |
| ![](data:image/x-wmf;base64...) | A specific relation type (e.g., treats, has\_property)                                                                                                     |
| ![](data:image/x-wmf;base64...) | An observed positive herb and an unobserved negative herb for a disease ![](data:image/x-wmf;base64...)                                                    |
| ![](data:image/x-wmf;base64...) | The ![](data:image/x-wmf;base64...)-dimensional latent representations of disease ![](data:image/x-wmf;base64...) and herb ![](data:image/x-wmf;base64...) |
| ![](data:image/x-wmf;base64...) | The predicted representation-based ranking score between ![](data:image/x-wmf;base64...) and ![](data:image/x-wmf;base64...)                               |

### Task Formulation

We formulate the prescription generation as an implicit feedback ranking task. Under the implicit feedback setting, the clinical dataset only provides observed positive interactions, indicating that an herb ![](data:image/x-wmf;base64...) is historically prescribed for a specific disease condition ![](data:image/x-wmf;base64...).

Our ultimate goal is to learn a robust representation-based scoring function ![](data:image/x-wmf;base64...), where ![](data:image/x-wmf;base64...) and ![](data:image/x-wmf;base64...) respectively denote the final ![](data:image/x-wmf;base64...)-dimensional latent representations of the disease and the herb learned by our model, and ![](data:image/x-wmf;base64...) denotes the inner product. The objective of the recommendation system is to optimize these representations such that the score of an observed positive herb ![](data:image/x-wmf;base64...) is consistently ranked higher than that of an unobserved negative herb ![](data:image/x-wmf;base64...) sampled from the candidate space ![](data:image/x-wmf;base64...). By successfully modeling this ranking boundary, the model can recommend a highly precise and pharmacologically sound subset of herbs for any given complex disease query.
...
|                   |           |             |             |          |         |            |            |            |              |              |               |
| ----------------- | --------- | ----------- | ----------- | -------- | ------- | ---------- | ---------- | ---------- | ------------ | ------------ | ------------- |
| **Method**        | **Top-K** | **Unified** | **Jaccard** | **Chem** | **SSL** | **P@10**   | **R@10**   | **F1@10**  | **DR\_P@10** | **DR\_R@10** | **DR\_F1@10** |
...
Overall, the results demonstrate that removing any of the aforementioned components leads to a decline in model performance. To ensure a clear and logical analysis, we categorize the discussion into structural interventions (RQ2) and semantic fusion mechanisms (RQ3).

**(1) Impact of Graph Topological Interventions (RQ2):**
The ablation of structural pruning components results in substantial performance degradation. Notably, removing the Top-K filter (W/O Top-K) significantly reduces performance, yielding the most severe drop (-59.18% in R@10 and -44.43% in F1@10). This confirms that without proper truncation, globally dense hub herbs easily dominate the graph message-passing, leading to over-smoothing. Similarly, decoupling the unified graph (W/O Unified) causes the second-worst degradation (-30.15% in F1@10), highlighting the necessity of processing heterogeneous entities within a unified topological space to preserve high-order multi-hop associations. Furthermore, replacing the Jaccard normalization with standard TF-IDF (W/O Jaccard) causes a noticeable drop, emphasizing that generic frequency penalization is suboptimal compared to our topology-aware metric.

**(2) Mitigation of Hub-node Bias and Semantic Over-smoothing (RQ2):**

To intuitively verify why the Top-K truncation prevents over-smoothing, we conducted an explicit long-tail distribution analysis. Based on global occurrence frequencies, the 395 herbs were stratified into three groups: Head (Top 20%, 79 ubiquitous herbs), Mid (20%-60%, 158 herbs), and Tail (Bottom 40%, 158 rare herbs). We evaluated the Recall@10 across these groups. As illustrated in Fig. X(a), standard bipartite models face a striking limitation: BSGAM completely collapses on the Tail group (Recall@10 = 0.0000) and acts as a generic "popularity recommender," heavily favoring ubiquitous herbs. KDHR maintains some Tail performance (0.1048), but falls behind in the Mid group. In contrast, HMC-GNN achieves a highly balanced distribution, drastically outperforming baselines in the Mid group (0.2449) and preserving strong Tail recommendation (0.1026).

To understand exactly how the asymmetric Top-K Jaccard graph construction achieves this balance, we compared it against two extreme topological variants: Dense Graph (retaining all meta-path connections without Top-K pruning) and No Edge (removing all collaborative edges). As depicted in Fig. X(b), the empirical data perfectly aligns with our hypothesis: the Dense Graph drives the Head Recall up to 0.5090 but degrades the Tail to 0.0724. This confirms that overly dense connections trigger severe topological over-smoothing, allowing structural hub nodes to contaminate rare local therapeutic signals. Conversely, the No Edge formulation isolates nodes, stripping away the essential collaborative generalization ability, which also results in poor Tail performance (0.0744). By acting as a critical topological bottleneck, our Top-K Jaccard strategy successfully filters out spurious hub connections while preserving high-confidence localized synergistic clusters, achieving the most balanced performance across the entire spectrum.

![](data:image/png;base64...)

Fig. 3. Long-tail distribution analysis and graph construction ablation. **(a)** Recall@10 performance comparison across Head, Mid, and Tail herb groups against baseline models. **(b)** Recall@10 performance under different topological graph construction strategies, demonstrating the necessity of the proposed Top-K information bottleneck.

**(3) Impact of Multimodal Fusion and Contrastive Alignment (RQ3):**
Alongside topological robustness, the incorporation of multimodal features and their fusion strategies heavily influences recommendation accuracy. As indicated in Table 4, completely omitting chemical formulations (W/O Chem) leads to a noticeable performance decline (-11.65% in F1@10), proving the value of integrating micro-level biochemical metrics.

To fully dissect the specific contributions of the proposed adaptive gated fusion (Eq. 6) and the quadruple joint optimization framework (Eq. 9), we conduct a fine-grained ablation study focusing on the multimodal fusion mechanisms, as shown in Table 5.

Table 5．

Fine-grained ablation study on Multimodal Fusion and Joint SSL objectives.

|                          |            |            |            |              |              |               |
| ------------------------ | ---------- | ---------- | ---------- | ------------ | ------------ | ------------- |
| **Method**               | **P@10**   | **R@10**   | **F1@10**  | **DR\_P@10** | **DR\_R@10** | **DR\_F1@10** |
| **HMC-GNN (Full Model)** | **0.1853** | **0.3435** | **0.1751** | -            | -            | -             |
...
First, replacing the Adaptive Gated Fusion with straightforward element-wise addition (W/O Gated Fusion) drops the F1@10 by 3.54%. This validates that heterogeneous modalities (structural vs. chemical) contribute differently across distinct herbs, making dynamic, node-specific feature gating crucial.

Moreover, decomposing the multi-task SSL framework reveals the necessity of our triplet contrastive design. Removing the Cross-Modal SSL or Prop-Chem Alignment results in noticeable degradations (-4.34% and -5.25% in F1@10, respectively). This proves that naively propagating features on the graph triggers "semantic collision" and modal drift, whereas explicit contrastive alignment successfully anchors the topological embeddings to their objective biochemical essence. Additionally, the Intra-Graph SSL introduces beneficial topological regularization, further enhancing model robustness against structural sparsity.
...
* Effect of Cutoff Size (Topological Information Bottleneck). In the heterogeneous collaborative graph, the hyperparameter dictates the strictness of the structural bottleneck. We fixed the co-occurrence out-degree on the disease side and varied the maximum out-degree of the herb-herb graph within . As illustrated in **Fig. 4(a)**, the performance curves (measured strictly by F1@5, F1@10, and F1@20) exhibit a distinct inverted-U shape, peaking precisely at . When is too small (e.g., ), the topology becomes overly sparse, leading to Information Fragmentation (isolated structural islands) that hinders the GNN from effectively propagating high-order synergistic signals. Conversely, an overly relaxed bottleneck () fails to filter out highly versatile hub herbs. Such indiscriminate dense connectivity triggers a severe Hairball Effect and message over-smoothing, rendering initially specific nodes indistinguishable in the latent space. Thus, strikes the optimal balance between preserving local semantic communities and suppressing generic graph noise.
* Effect of Independent Contrastive Learning Weights. To dissect the multi-task trade-off, we explicitly decouple and investigate the three auxiliary self-supervised learning weights: the intra-graph SSL (![](data:image/x-wmf;base64...)), the cross-modal distillation (![](data:image/x-wmf;base64...)), and the property-chemical alignment (![](data:image/x-wmf;base64...)). During evaluation, we varied each hyperparameter independently while keeping the others fixed at their respective optimal values. As depicted in Figure 3(b), the model achieves peak performance at ![](data:image/x-wmf;base64...). A moderate intra-graph SSL weight effectively prevents topological collapse against structural perturbation; however, excessive values (![](data:image/x-wmf;base64...)) force the model to focus purely on reconstructing the graph topology, overshadowing the primary BPR recommendation objective. Similarly, in Figure 3(c), the cross-modal alignment performs optimally at ![](data:image/x-wmf;base64...). This ensures that topological embeddings softly absorb objective chemical attributes without suffering from semantic collision, whereas a large ![](data:image/x-wmf;base64...) incorrectly forces graph representations to strictly mimic rigid biochemical spaces, harming its collaborative capabilities. Finally, Figure 3(d) explores the property-chemical prior alignment. It is most effective at a slightly lower magnitude (![](data:image/x-wmf;base64...) or 0.1), serving as a gentle semantic anchor between macroscopic behaviors and microscopic metrics without overriding the interaction network-derived synergies.
* Effect of Temperature Coefficient (Contrastive Hardness). The temperature scaling parameter in the InfoNCE loss controls the penalty strength assigned to hard negative samples during cross-modal alignment. We evaluated . As shown in **Fig. 4(c)**, the model yields optimal metrics at . An excessively high (e.g., ) causes the model to treat all negative samples equally, leading to an overly uniform feature distribution that blurs the distinct pharmacological boundaries (e.g., failing to geometrically separate Bitter and Pungent properties). Conversely, an extremely low forces the model to be overly sensitive to noise and local anomalous structures, resulting in training instability and poor overall generalization.
* Effect of Latent Embedding Dimension (Model Capacity). The embedding dimension determines the expressive capacity of the unified latent space. We tested . As presented in **Fig. 4(d)**, performance consistently improves as increases from 32 to 128. This demonstrates that higher dimensions are required to sufficiently accommodate both the sparse graph topological structure and the rich, pre-trained chemical semantics (projected from an initial 768-dimensional space). However, beyond , the performance gains plateau and begin to slightly degrade. We attribute this to model over-parameterization, which inevitably increases the risk of overfitting on the limited, high-sparsity disease-herb interaction pairs.

![](data:image/png;base64...)

Fig. 4. Hyperparameter sensitivity analysis of the proposed HMC-GNN framework. **(a)** the asymmetric topological cutoff size ; **(b)** the overall contrastive learning weight ; **(c)** the InfoNCE temperature coefficient ; and **(d)** the latent embedding dimension
...
| Type 2 Diabetes  （消渴症） | Brucea Fruit (鸦胆子),  Achyranthes Root (牛膝),  Red Ginseng (红参),  Reishi Mushroom (灵芝),  Lycopus (泽兰)                                                                              | **Brucea Fruit** (鸦胆子),  **Achyranthes Root** (牛膝),  **Red Ginseng** (红参),  **Reishi Mushroom** (灵芝),  **Lycopus** (泽兰)                                                                                                  | Clear Heat and Generate Fluid.  Activate Blood and Tonify Liver/Kidney.  Greatly Tonify Primal Qi.  Replenish Qi and Nourish Strength  Activate Blood and Resolve Stasis.                                                                                                                                                                                                                                                      |
| Precision@5: 1.0000    |                                                                                                                                                                                  |                                                                                                                                                                                                                          |                                                                                                                                                                                                                                                                                                                                                                                                                                |
| **Case2**              |                                                                                                                                                                                  |                                                                                                                                                                                                                          |                                                                                                                                                                                                                                                                                                                                                                                                                                |
| Disease (TCM Syndrome) | **Herb Set**                                                                                                                                                                     |                                                                                                                                                                                                                          |                                                                                                                                                                                                                                                                                                                                                                                                                                |
| Ground Truth           | HMC-GNN                                                                                                                                                                          | Effects                                                                                                                                                                                                                  |                                                                                                                                                                                                                                                                                                                                                                                                                                |
| Cough  （咳嗽）            | Rehmannia (地黄),  Asparagus Root (天冬),  Cornus (山茱萸),  Chinese Yam (山药),  Jujube (大枣),  Atractylodes (白术),  Purslane (马齿苋),  Cocklebur (苍耳子),  Typhaceae (蒲黄)\*,  Codonopsis (党参) | **Rehmannia** (地黄),  **Asparagus Root** (天冬),  **Cornus** (山茱萸),  **Chinese Yam** (山药),  **Jujube** (大枣),  **Atractylodes** (白术),  **Purslane** (马齿苋),  **Cocklebur** (苍耳子),  Pokeweed Root (商陆)\*,  **Codonopsis** (党参) | Clear Heat and Generate Fluid.  Nourish Yin, Moisten Lungs and Generate Fluid.  Tonify Liver and Kidney.  Generate Fluid, Benefit Lungs and Tonify Spleen.  Tonify Spleen/Stomach and Moisten Lungs.  Tonify Spleen Qi and Resolve Phlegm/Dampness.  Clear Heat and Detoxify.  Dispel Wind-Cold and Open Nasal Passages.  Dispel Water Retention and Dissipate Nodules.  Tonify Spleen, Benefit Lungs and Replenish Middle Qi. |
| Precision@10: 0.9000   |                                                                                                                                                                                  |                                                                                                                                                                                                                          |                                                                                                                                                                                                                                                                                                                                                                                                                                |
